...
test("vendor emits only 4/5/6 link-count patterns plus retained filters", async () => {
  ...
  assert.doesNotMatch(result.primary, /\(-\\w\)\{1\}|\(-\\w\)\{2\}/);
...
test("vendor source and output contain no color-link or six-socket paths", async () => {
  ...
  for (const forbidden of [
    "anyTwoLink", "anyThreeLink", "anyTwoColorLink", "anyThreeColorLink",
